...
export default function AdminBrandsPage() {
  ...
  async function load() {
    ...
    const { data: products } = await supabase.from("products").select("product_brand_id");
  ...
  const filtered = useMemo(() => {
    ...
    if (!x) return rows;
    return rows.filter((b) => (b.name || "").toLowerCase().includes(x));
  }, [rows, q]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "var(--nomi-muted)" }}>
          {filtered.length} de {rows.length} marca{rows.length !== 1 ? "s" : ""}
        </p>
      </div>

      {msg && !editing && (
        <div className="px-4 py-3 rounded-xl text-sm font-semibold"
          style={msg.ok ? { backgroundColor: "#DCFCE7", color: "#16A34A" } : { backgroundColor: "#FEE2E2", color: "#DC2626" }}>
          {msg.text}
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="col-span-3 py-12 text-center">
            ...
            <p className="text-sm font-semibold" style={{ color: "var(--nomi-muted)" }}>
              {q ? `No hay marcas que coincidan con "${q}"` : "No hay marcas registradas aun"}
            </p>
          </div>
        ) : filtered.map((b) => (
          <div key={b.id} className="bg-white rounded-2xl overflow-hidden" style={{ border: "1.5px solid var(--nomi-border)" }}>
            <div className="h-20 flex items-center justify-center p-4" style={{ backgroundColor: "var(--nomi-gray)" }}>
              {(editing === b.id ? editLogo : b.logo_url) ? (
                <img src={editing === b.id ? editLogo : b.logo_url} className="h-14 object-contain" alt={b.name}
            ...
            <div className="p-4 space-y-3">
              {editing === b.id ? (
                <>
                  <div>
                    <label className="block text-xs font-bold mb-1 uppercase tracking-wide" style={{ color: "var(--nomi-navy)" }}>Nombre</label>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditing(null)} className="px-3 py-2 rounded-xl cursor-pointer"
                ...
                <>
                  <div>
                    <div className="font-black text-sm" style={{ color: "var(--nomi-navy)" }}>{b.name || "Sin nombre"}</div>
                    {sellerMap[b.seller_brand_id] && (
                      <div className="text-xs mt-0.5" style={{ color: "var(--nomi-muted)" }}>Proveedor: {sellerMap[b.seller_brand_id]}</div>
                    )}
