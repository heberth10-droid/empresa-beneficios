...
function CartPageContent() {
  ...
  if (items.length === 0) return (
    <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-5">
      ...
      <p className="text-sm" style={{ color: "var(--nomi-muted)" }}>Agrega productos o cursos para continuar</p>
      <div className="flex gap-3 justify-center">
        <Link href="/market"
          className="px-6 py-3 rounded-xl text-sm font-black cursor-pointer"
          style={{ backgroundColor: "var(--nomi-orange)", color: "#fff" }}>
          Ver productos
        </Link>
        <Link href="/market/courses"
          className="px-6 py-3 rounded-xl text-sm font-black cursor-pointer"
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "var(--nomi-muted)" }}>{items.length} {items.length === 1 ? "item" : "items"}</p>
        </div>
            ...
            <div key={it.id} className="bg-white rounded-2xl p-4 flex gap-4 items-center"
              ...
              {it.isCourse ? (
                <div className="w-20 h-20 rounded-xl flex items-center justify-center shrink-0"
                  style={{ backgroundColor: "#EDE9FE", border: "1px solid var(--nomi-border)" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {it.isCourse && (
                  <p className="text-xs mt-2" style={{ color: "var(--nomi-muted)" }}>
                    Recibes acceso en 8 dias por correo
                  </p>
                )}
              </div>
              ...
              <div className="text-right shrink-0 space-y-2">
                ...
                <button onClick={() => removeItem(it.id)} className="cursor-pointer" style={{ color: "#DC2626" }}>
        ...
        <div>
          <div className="bg-white rounded-2xl p-5 space-y-4 md:sticky md:top-24"
            ...
            <div className="space-y-2">
              {items.map((it) => (
                <div key={it.id} className="flex justify-between text-sm">
                  <span style={{ color: "var(--nomi-muted)" }}>
                    {it.name}{it.isCourse ? " (Curso)" : ` x${it.qty}`}
                  </span>
            ...
            <div className="text-xs text-center space-y-1" style={{ color: "var(--nomi-muted)" }}>
              <p>✓ 0% intereses · Descuento por nomina</p>
              <p>✓ Cursos: acceso en 8 dias por correo</p>
            </div>
          </div>
        </div>
      </div>
